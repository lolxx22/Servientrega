...
const Tip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background: 'white', padding: '8px 12px', borderRadius: 10, border: '1px solid #F1F3F5', boxShadow: '0 4px 16px rgba(0,0,0,0.10)' }}>
      <p style={{ fontSize: 12, fontWeight: 600, color: '#1A202C', margin: '0 0 4px' }}>{label}</p>
      {payload.map((it: any, i: number) => (
        <p key={i} style={{ fontSize: 12, color: '#6C7681', margin: 0 }}>{it.name}: {it.value}</p>
      ))}
    </div>
  );
};
  ...
  return (
    <div style={CARD}>
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={chartData.shipmentsByCity} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
          ...
          <YAxis
            ...
            axisLine={false} tickLine={false}
          />
          <Tooltip content={<Tip />} cursor={{ fill: 'rgba(30,138,76,0.04)' }} />
          <Bar
            dataKey="count" name="Envíos"
